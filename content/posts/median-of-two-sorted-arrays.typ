...
看到有序 + $O(log n)$ 复杂度，显然这是一个二分查找的题，但是难点就在于这里同时有两个数组，如果做归并（merge）并提前返回，时间复杂度一定是 $O(m + n)$ 的，所以我们必须在不合并的前提下同时对两个数组做二分查找。
...
对于 $forall k in [0..n + m - 1] inter ZZ$，我们要找的数肯定要么位于 $a$，要么位于 $b$，要么作为重复元素同时位于 $a$ 和 $b$。不管怎样，我们都可以假设，在通过二分查找到索引为 $k$ 的数时，在 $a$ 中至少有 $k_1$ 个数小于等于这个数，在 $b$ 中至少有 $k_2$ 个数小于等于这个数，则有 $k_1 + k_2 = k$。

证明很简单，如果我们在合并后的数组 $c$ 中根据索引 $k$ 将其切成两半，那么左边正好有 $k$ 个数。由于 $a$ 和 $b$ 各自有序，这个切分对应到两个数组中也一定是两个前缀，设这两个前缀长度分别为 $k_1$ 和 $k_2$，自然有 $k_1 + k_2 = k$。
