...
对于 $forall k in [0..n + m - 1] inter ZZ$，我们要找的数肯定要么位于 $a$，要么位于 $b$，要么作为重复元素同时位于 $a$ 和 $b$。不管怎样，我们都可以假设，在最后我们二分的时候找到的这个数，如果插入到 $a$ 和 $b$ 中，它所插入的索引分别为 $k_1$ 和 $k_2$，则有 $k_1 + k_2 = k$。

证明很简单，两个数组有序，因此新数插入时的索引值即为前面有多少个小于该数的数，因此 $k_1$ 和 $k_2$ 分别代表在 $a$ 和 $b$ 两个数组中小于该数的数的数量，两者相加即为在合并后的 $c$ 中小于该数的数的数量，也就是索引为 $k$ 的数。

举个例子，$a = [1, 3, 5], b = [2, 4, 6]$，我找的是 $k = 3$ 的元素，归并后找一下发现是 $4$ 这个数。如果我把 $4$ 插入到 $a$ 中的话，需要插入到 $k_1 = 2$ 的位置上去；而插入到 $b$ 中的话，则需要插入到 $k_2 = 1$ 的位置上去。因此有 $k_1 + k_2 = k$。

我们二分的时候所需要找的就是合适的的 $k_1$ 和 $k_2$，正好对应着我们要找的索引为 $k$ 的元素被插入到两个数组时的两个索引，而其中 $k_1$ 或者 $k_2$ 所在索引处正好对应着我们需要的索引为 $k$ 的元素，即 $c[k] = min(a[k_1], b[k_2])$。
...
但是，我们怎么知道什么情况下 $k_1$ 和 $k_2$ 恰好是 $c[k]$ 的插入索引呢？这时候，我们就要思考一下如果恰好是的情况下，会有什么特性：

- 两数组有序，因此显然 $a[k_1 - 1] <= a[k_1]$ 且 $b[k_1 - 1] <= b[k_1]$。
- 假设 $a[k_1] = c[k]$，那么肯定有 $b[k_2] >= a[k_1 - 1]$，否则，如果 $b[k_2] < a[k_1 - 1]$，则有 $b[k_2] < a[k_1 - 1] <= a[k_1] = c[k]$，那么在 $b$ 中，比 $c[k]$ 小的数至少有 $k_2 + 1$ 个（从 $b[0]$ 到 $b[k_2]$），而在 $a$ 中，比 $c[k]$ 小的数有 $k_1$ 个（从 $a[0]$ 到 $a[k_2 - 1]$），因此在 $c$ 中，比 $c[k]$ 小的数至少有 $k_1 + k_2 + 1$ 个，但是 $k_1 + k_2 = k$，矛盾！
- 假设 $b[k_1] = c[k]$，那么同理，也肯定有 $a[k_1] >= b[k_2 - 1]$。

纯数学语言可能不是很直观，画出来就很直观了。

当 $a[k_1] = c[k]$ 时，如果 $b[k_2] < a[k_1 - 1]$，那么这种错误情况下的*错误*排列如下所示：
...
当 $b[k_1] = c[k]$ 时，如果 $a[k_1] < b[k_2 - 1]$，那么这种错误情况下的*错误*排列如下所示：
...
而 $k_1$ 和 $k_2$ 恰好是 $c[k]$ 的插入索引时对应的 $a$ 和 $b$ 数组中元素的*正确*排列方式为：
...
其实，在这种解法之上，其实我们还可以再做一些优化，让 `findKth` 只需要调用一次！

注意到，我们在使用二分查找的时候，额外选用的是 $k_1 - 1$ 和 $k_2 - 1$ 这两个左边的索引，那么显然，如果我用 $k_1 + 1$ 和 $k_2 + 1$ 这两个右边的索引，然后把函数里面对应的符号修改一下，是不是也是一样的能够实现 `findKth` 函数？

再进一步，实际上 `findKth` 函数不仅找到了索引为 $k$ 的数，同时还找到了索引为 $k - 1$ 的数！即，$c[k] = min(a[k_1], b[k_2])$，且 $c[k - 1] = max(a[k_1 - 1], a[k_2 - 1])$！
...
这是一道非常经典并且有趣的二分查找的困难题，很久之前就在我校《算法基础》课程的考试中出现过，当时我实在没想出来有什么好的解法，直到最近刷 leetcode 才想到如此优雅快捷并且易于理解的好方法，希望能帮助读者更好吃透这道优秀的题目。
